/*
 * @file /parastation-core/src/spu/interpolate.rs
 * @brief
 * Gaussian interpolation table and function for sound sample interpolation in the PS1 SPU.
...
pub fn interpolate_gaussian(samples: &[PcmSample; 4], pitch_counter: u16) -> PcmSample {
    ...
    */
    let mut interpolated = (GAUSSIAN_TABLE[0x0FF - idx] * s[0]) >> 15;
    interpolated += (GAUSSIAN_TABLE[0x1FF - idx] * s[1]) >> 15;
    interpolated += (GAUSSIAN_TABLE[0x100 + idx] * s[2]) >> 15;
    interpolated += (GAUSSIAN_TABLE[idx] * s[3]) >> 15;

    PcmSample(interpolated as i16)
}
