...
macro_rules! bus_read {
    ($self:expr, $addr:expr, $method:ident, $width:expr) => {{
        ...
        if let Some(offset) = RAM.contains(addr) { return $self.ram.$method(offset); }
        if let Some(offset) = BIOS.contains(addr) { return $self.bios.$method(offset); }
        if let Some(offset) = SCRATCHPAD.contains(addr) { return $self.scratchpad.$method(offset); }
...
macro_rules! bus_write {
    ($self:expr, $addr:expr, $value:expr, $method:ident, $width:expr) => {{
        ...
        if let Some(offset) = RAM.contains(addr) { return $self.ram.$method(offset, $value); }
        if let Some(offset) = SCRATCHPAD.contains(addr) { return $self.scratchpad.$method(offset, $value); }
        if let Some(_)      = EXP2.contains(addr) { return; }
...
impl SystemBus {
    fn read_hardware(&self, addr: u32) -> u32 {
        if let Some(_offset) = EXP1.contains(addr) { return 0xFF; }
        if let Some(offset) = MEMORY_CONTROL_1.contains(addr) { return self.read_memory_control_1(offset); }
        if addr == 0x1F80_1060 { return 0x0000_0B88; } // RAM_SIZE
        if let Some(offset) = INTERRUPT_CONTROL.contains(addr) { return self.read_interrupt_control(offset); }
        if let Some(offset) = DMA_REGISTERS.contains(addr) { return self.dma.read_register(offset); }
        if let Some(_offset) = TIMERS.contains(addr) { return 0; } // TODO timers
        if let Some(offset) = GPU_REGISTERS.contains(addr) { return self.read_gpu_register(offset); }
        if let Some(_offset) = SPU_REGISTERS.contains(addr) { return 0; } // TODO spu
    ...
    fn write_hardware(&mut self, addr: u32, value: u32, width: AccessWidth) {
        ...
        if let Some(_offset) = MEMORY_CONTROL_1.contains(addr) { return; } // Absorb writes to memory control 1, just gonna hardware. HACK
        if addr == 0x1F80_1060 { return; } // Absorb writes to RAM_SIZE
        if let Some(offset) = INTERRUPT_CONTROL.contains(addr) { return self.write_interrupt_control(offset, value as u16); }
        if let Some(offset) = DMA_REGISTERS.contains(addr) { return self.dma.write_register(offset, value); }
        if let Some(_offset) = TIMERS.contains(addr) { return; } // TODO timers
        if let Some(offset) = GPU_REGISTERS.contains(addr) { return self.write_gpu_register(offset, value); }
        if let Some(_offset) = SPU_REGISTERS.contains(addr) { return; } // TODO spu

        if addr == 0xFFFE_0130 { return; } // Absorb writes to the cache control, doesn't matter. HACK
    ...
    fn read_memory_control_1(&self, offset: u32) -> u32 {
        ...
        match offset {
            ...
            _ => { eprintln!("Unhandled read from memory control 1 at offset {offset:#x}"); 0 }
        }
    }
    ...
    fn read_interrupt_control(&self, offset: u32) -> u32 {
        match offset {
            ...
            _ => { eprintln!("Unhandled read from interrupt controller at offset {offset:#x}"); 0 }
        }
    }
    ...
    fn write_interrupt_control(&mut self, offset: u32, value: u16) {
        match offset {
            ...
            _ => eprintln!("Unhandled write to interrupt controller at offset {offset:#x} with value {value:#x}"),
        }
    }
...
impl SystemBus {
    pub fn read32(&self, addr: u32) -> u32   { bus_read!(self, addr, read32, AccessWidth::Word) }
    pub fn read16(&self, addr: u32) -> u16   { bus_read!(self, addr, read16, AccessWidth::Half) }
    pub fn read8 (&self, addr: u32) -> u8    { bus_read!(self, addr, read8,  AccessWidth::Byte) }
    pub fn write32(&mut self, addr: u32, value: u32)  { bus_write!(self, addr, value, write32, AccessWidth::Word) }
    pub fn write16(&mut self, addr: u32, value: u16)  { bus_write!(self, addr, value, write16, AccessWidth::Half) }
    pub fn write8 (&mut self, addr: u32, value: u8)   { bus_write!(self, addr, value, write8,  AccessWidth::Byte) }
}
...
impl SystemBus {
    ...
    fn dma_gpu_linked_list(&mut self, base_addr: u32) {
        ...
        loop {
            ...
            // End marker: Bit 32 set or all lower 24 bits set
            let next = header & 0x00FF_FFFF;
            if next >= 0x00FF_FFFF || (header & 0x8000_0000) != 0 {
                break;
            }

            addr = next & 0x001F_FFFF; // Wrap around the 2MB RAM size
        }
    }
    ...
    fn dma_gpu_vram_write(&mut self, base_addr: u32, word_count: u32) {
        ...
        for _ in 0..word_count {
            ...
            addr = (addr + 4) & 0x001F_FFFF; // Wrap around the 2MB RAM size
        }
    }
...
impl SystemBus {
    fn execute_dma_transfer(&mut self, transfer: DmaTransfer) {
        match transfer {
            ...
            DmaTransfer::GpuVramWrite { src_addr, word_count } => self.dma_gpu_vram_write(src_addr, word_count),
            DmaTransfer::GpuVramRead { dest_addr, word_count } => self.dma_gpu_vram_read(dest_addr, word_count),
